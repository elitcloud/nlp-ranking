...
def get_entry_dict(bib_map, bib_dir, discard_abstract=True):
    """
    ...
    def valid(entry, weight):
        if discard_abstract and weight == 1.0:
            if 'pages' in entry:
...
def generate_email_map(bib_map_file, bib_dir, txt_dir, email_map_file):
    print('Reading: ' + bib_map_file)
    bib_map = load_map(bib_map_file)
    print('Reading: ' + bib_dir)
    entry_dict = get_entry_dict(bib_map, bib_dir, False)
    print('Reading: ' + txt_dir)
    email_dict = get_email_dict(txt_dir)
    print('Writing: ' + email_map_file)
...
def load_email_map(email_map_file):
    fin = open(email_map_file)
